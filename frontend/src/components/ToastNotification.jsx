...
export const useToastNotification = () => {
	...
	const showSuccess = (title, description) => {
		const id = Date.now();
		setToasts(prev => [...prev, { id, type: 'success', title, description }]);
		setTimeout(() => removeToast(id), 5000);
	};

	const showError = (title, description) => {
		const id = Date.now();
		setToasts(prev => [...prev, { id, type: 'error', title, description }]);
		setTimeout(() => removeToast(id), 7000);
	};

	const showWarning = (title, description) => {
		const id = Date.now();
		setToasts(prev => [...prev, { id, type: 'warning', title, description }]);
		setTimeout(() => removeToast(id), 5000);
	};

	const showInfo = (title, description) => {
		const id = Date.now();
		setToasts(prev => [...prev, { id, type: 'info', title, description }]);
		setTimeout(() => removeToast(id), 5000);
	};
